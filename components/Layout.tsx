...
export const Layout: React.FC<LayoutProps> = ({ children, user, onLogout, title, onDashboard }) => {
  ...
  const handleNavClick = (e: React.MouseEvent, href: string) => {
    if (href.startsWith('#')) {
      ...
      // Allow React to render the dashboard view if needed, then scroll
      setTimeout(() => {
  ...
  const NavItem = ({ icon: Icon, label, href }: { icon: any, label: string, href: string }) => (
    <a 
      ...
      className="group flex items-center space-x-3 px-6 py-4 text-stone-600 dark:text-stone-400 hover:text-amber-600 dark:hover:text-amber-400 rounded-2xl transition-all duration-300 hover:bg-white/50 dark:hover:bg-black/20 hover:shadow-lg hover:shadow-amber-500/10 cursor-pointer"
    >
